...
export default function Rerun() {
  ...
  // Marking an episode watched checks for earlier unwatched episodes of the same
  // show first, and asks whether to mark those too, instead of silently skipping them.
  const requestMarkWatched = (show, ep) => {
    const alreadyWatched = !!data.watchedLog[ep.id];
    if (alreadyWatched) { toggleEpisodeWatched(show, ep); return; }
    const eps = episodesCache[show.id] || [];
    const prior = eps.filter(e =>
      !data.watchedLog[e.id] && e.id !== ep.id &&
      (e.season < ep.season || (e.season === ep.season && e.number < ep.number))
    );
    if (prior.length > 0) setConfirmDialog({ show, ep, prior });
    else toggleEpisodeWatched(show, ep);
  };
    ...
    <div className="rr-root">
      ...
      <div className="rr-frame">
        ...
        <div className="rr-content">
          {!detail && nav === "shows" && (
            <ShowsHome data={data} episodesCache={episodesCache}
              ...
              onQuickWatch={requestMarkWatched} />
          )}
          {!detail && nav === "movies" && (
          ...
          {detail?.type === "show" && (
            <ShowDetail
              ...
              onToggleEp={requestMarkWatched} onSetSeasonWatched={setSeasonWatched}
        ...
        <nav className="rr-nav">
          <NavBtn icon={Tv} label="Shows" active={nav === "shows"} onClick={() => { setDetail(null); setNav("shows"); }} />
          <NavBtn icon={Film} label="Movies" active={nav === "movies"} onClick={() => { setDetail(null); setNav("movies"); }} />
          <NavBtn icon={Compass} label="Explore" active={nav === "explore"} onClick={() => { setDetail(null); setNav("explore"); }} />
          <NavBtn icon={BarChart3} label="Stats" active={nav === "stats"} onClick={() => { setDetail(null); setNav("stats"); }} />
          <NavBtn icon={Sparkles} label="AI" active={nav === "ai"} onClick={() => { setDetail(null); setNav("ai"); }} />
        </nav>
      </div>

      {confirmDialog && (
        <div className="rr-overlay" onClick={() => setConfirmDialog(null)}>
          <div className="rr-dialog" onClick={e => e.stopPropagation()}>
            <div className="rr-dialog-title">Mark earlier episodes too?</div>
            <div className="rr-dialog-body">
              You have {confirmDialog.prior.length} earlier unwatched episode{confirmDialog.prior.length > 1 ? "s" : ""} of {confirmDialog.show.name}. Mark {confirmDialog.prior.length > 1 ? "them" : "it"} as watched too?
            </div>
            <div className="rr-dialog-actions">
              <button className="rr-btn outline" onClick={() => {
                toggleEpisodeWatched(confirmDialog.show, confirmDialog.ep);
                setConfirmDialog(null);
              }}>Just this one</button>
              <button className="rr-btn primary" onClick={() => {
                setSeasonWatched(confirmDialog.show, [...confirmDialog.prior, confirmDialog.ep], true);
                setConfirmDialog(null);
...
function GroupSection({ label, grid, children, sectionRef }) {
  return (
    <div className="rr-group" ref={sectionRef}>
...
function ExploreHome({ data, onOpenShow, onOpenMovie, onAddShow, onGoToMovies }) {
  ...
  // "Top recommended for you": rank the trending pool by overlap with genres
  // you've actually watched, instead of showing the same list as raw "trending".
  const recommended = useMemo(() => {
    ...
    return trending
      .filter(s => !data.shows[s.id])
      .map(s => ({ show: s, overlap: (s.genres || []).filter(g => topGenres.includes(g)).length }))
      .filter(s => s.overlap > 0)
      .sort((a, b) => b.overlap - a.overlap || b.show.rating.average - a.show.rating.average)
      .slice(0, 6)
      .map(s => s.show);
        ...
        !trending ? (
          ...
          <>
            {recommended.length > 0 ? (
              <>
                <div className="rr-section-label">Top recommended for you</div>
              ...
              <EmptyState icon={Sparkles} text="Recommendations unlock as you watch." sub="Mark a few episodes watched and this section will personalize." />
            )}
            ...
            <div className="rr-moviecta">
              <div className="rr-moviecta-text">Movie discovery needs a licensed catalog we don't have free access to. Add movies you want to track by hand instead.</div>
...
/* ============ EPISODE DETAIL ============ */
function EpisodeDetail({ showId, episodeId, data, episodesCache, onBack, onSave, onMarkWatched }) {
  const eps = episodesCache[showId] || [];
  ...
  return (
    <div className="rr-view">
      ...
      <button className={`rr-bigcheck ${watched ? "on" : ""}`} onClick={() => onMarkWatched(showObj, ep)}>
...
function Style() {
  return (
    <style>{`
      ...
      .rr-moviecta { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:14px; display:flex; flex-direction:column; gap:10px; margin-bottom:10px; }
      .rr-moviecta-text { font-size:12px; color:var(--text-muted); line-height:1.5; }

      .rr-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.55); display:flex; align-items:center; justify-content:center; z-index:50; padding:20px; }
      .rr-dialog { background:var(--surface); border:1px solid var(--border); border-radius:14px; padding:18px; max-width:340px; width:100%; }
      .rr-dialog-title { font-family:'Archivo Black',sans-serif; font-size:15px; margin-bottom:8px; }
      .rr-dialog-body { font-size:13px; color:var(--text-muted); line-height:1.5; margin-bottom:16px; }
