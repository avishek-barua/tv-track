...
## Update — production data loss after idle restart
User reported shows disappearing in production specifically after the Render service woke back up from idle sleep (worked fine locally). Diagnosed as Render's free-tier ephemeral filesystem: unlike a real persistent disk (a paid Render add-on), the free tier's local filesystem isn't guaranteed to survive a spin-down/spin-up cycle, so `db.json` was getting wiped exactly when the service restarted from idle — matching the symptom precisely.

Fix, staying within the "free" constraint: added optional Upstash Redis support to `server.mjs`. It auto-detects `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` env vars — if present, it stores the same single JSON blob in Redis (`SET`/`GET` one key) instead of the local file; if absent, it falls back to `db.json` exactly as before, so local dev needs no signup and no changes. Documented the two-env-var Upstash setup in `DEPLOY.md` (free tier, no card, doesn't sleep, persists indefinitely — plenty for one person's watch history).

## Update — swapped Redis for real Postgres
User asked for "a real postgresql free service" instead — more realistic/standard than Redis for this kind of app. Swapped `server.mjs`'s backend from Upstash Redis to Postgres via `pg`, keeping the exact same auto-detect pattern: `DATABASE_URL` present → Postgres (a single `rerun_data` table, one JSONB row, upserted whole on every save — same "one blob" model as everything else in this app); absent → local `db.json` file, unchanged for local dev.

Picked **Neon** as the free Postgres host specifically because its free-tier compute auto-suspends when idle but resumes automatically on the next query with no manual step — Supabase's free tier was considered and ruled out for this use case because it fully pauses a project after a week of inactivity and requires clicking "restore" in its dashboard before it'll respond again, which would reintroduce a version of the same "silently broken until you notice" problem this whole thread has been chasing down.

Updated `DEPLOY.md`, `SETUP.md` (added `pg` to the local install command, since the import is unconditional even though the connection itself is optional), `CLAUDE.md`, and this file to say Postgres/Neon instead of Upstash throughout.
